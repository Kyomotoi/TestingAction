...
request_friend = sv.on_command(
    cmd="friendreq", aliases={"fr"}, docs=__doc__, permission=SUPERUSER
)
...
request_group = sv.on_command(
    cmd="groupreq", aliases={"gr"}, docs=__doc__, permission=SUPERUSER
)
    ...
    if key == "list":
        ...
        try:
            await bot.set_group_add_request(
                flag=arg, sub_type=data[arg]["sub_type"], approve=False
            )
            await request_friend.finish(f"完成~！已同意 {data[arg]['user_id']} 的申请")
        except:
...
broadcast = sv.on_command(
    cmd="boradcast", aliases={"bc"}, docs=__doc__, permission=SUPERUSER
)
...
track_error = sv.on_command(cmd="track", docs=__doc__, permission=SUPERUSER)
...
get_log = sv.on_command(
    cmd="getlog", aliases={"gl"}, docs=__doc__, permission=SUPERUSER
)
...
shutdown = sv.on_command(
    cmd="shutdown", aliases={"st"}, docs=__doc__, permission=SUPERUSER
)
...
service_control = sv.on_command(
    cmd="service", aliases={"s"}, docs=__doc__, permission=SUPERUSER
)
